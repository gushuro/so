...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number) {
	uint bg = blockgroup_for_inode(inode_number);
	uint offset = blockgroup_inode_index(inode_number);
	Ext2FSBlockGroupDescriptor* bg_descriptor = block_group(bg);
	uint itable = bg_descriptor->inode_table;
	unsigned int block_size = 1024 << superblock()->log_block_size;
	uint inodes_per_block = block_size/superblock()->inode_size;
	unsigned char* buf = (unsigned char*)malloc(block_size);
	uint myblock = (offset / inodes_per_block) + itable;
	uint myindex = (offset % inodes_per_block)*superblock()->inode_size;
	read_block(myblock, buf);
	Ext2FSInode * inodo_bonito = (Ext2FSInode*)malloc(superblock()->inode_size);
	*inodo_bonito = *((struct Ext2FSInode *) (buf+myindex));
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number) {
	unsigned int block_size = 1024 << _superblock->log_block_size;
	unsigned int cantpunteros = block_size / 4; // sizeof(unsigned int);
	if (block_number <= 11) {
		return inode->block[block_number];
	} else if (block_number < 12 + cantpunteros  ) {
		unsigned char* buf = (unsigned char*)malloc(block_size);
		unsigned int primeraind = inode->block[12];
		read_block(primeraind, buf);
		unsigned int res = ((unsigned int*)buf)[block_number-12];
		free(buf);
		return res;
	} else if (block_number < 12 + cantpunteros + cantpunteros*cantpunteros) {
		unsigned char *buf = (unsigned char*)malloc(block_size);
		unsigned int primeraind = inode->block[13];
		read_block(primeraind, buf);
		unsigned int relative = block_number - 12 - cantpunteros;
		unsigned int bl = ((unsigned int*)buf)[relative / cantpunteros];
		unsigned int index = relative % cantpunteros;
		read_block(bl, buf);
		unsigned int res = ((unsigned int*)buf)[index];
		free(buf);
		return res;
	} else {
		unsigned char *buf = (unsigned char*)malloc(block_size);
		unsigned int primeraind = inode->block[14];
		read_block(primeraind, buf);
		unsigned int relative = block_number - 12 - cantpunteros - cantpunteros*cantpunteros;
		unsigned int bl = ((unsigned int*)buf)[relative / (cantpunteros*cantpunteros)];
		unsigned int findex = (relative % (cantpunteros*cantpunteros))/ cantpunteros;
		unsigned int sindex = relative % cantpunteros;
		read_block(bl, buf);
		unsigned int sbl = ((unsigned int*)buf)[findex];
		read_block(sbl, buf);
		unsigned int res = ((unsigned int*)buf)[sindex];
		free(buf);
		return res;
	}
}
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	unsigned int block_size = 1024 << _superblock->log_block_size;
	uint size = from->size;
	uint cant_blocks = from->blocks;

	unsigned char *buf = (unsigned char*)malloc(cant_blocks*block_size);
	for (int i = 0; i < cant_blocks; ++i){
		read_block(get_block_address(from, i), (unsigned char*)((unsigned int)(buf)+(i*block_size)));
	}
	...
	// chururun
	while(index < size){
		Ext2FSDirEntry *entry = (Ext2FSDirEntry*)((unsigned int)(buf)+(index));
		if (strcmp(entry->name, filename)==0) {
			free(buf);
			return load_inode(entry->inode);
		}
		index += entry->record_length;
	}
	free(buf);
	return NULL;
